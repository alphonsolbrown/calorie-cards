# fdc_lookup.py
"""
USDA FoodData Central calorie lookup with support for "each" and "half".
- Prefers real USDA portion gram weights (foodPortions).
...
def _get_cal_per_100g(food_details: Dict[str, Any]) -> Optional[float]:
    for n in food_details.get("foodNutrients", []):
        if _safe_get(n, "nutrient", "number") == "1008":  # kcal
            amount = n.get("amount")
...
def _find_portion_matches(food_details: Dict[str, Any]) -> List[Tuple[str, float]]:
    ...
    for p in food_details.get("foodPortions", []) or []:
        ...
        if mu and mu.lower() not in desc.lower(): pieces.append(mu)
        description = ", ".join([s for s in pieces if s])
        if gw and description:
            portions.append((description.lower(), float(gw)))
    return portions
...
def _portion_for_measure(measure: str, portions: List[Tuple[str, float]]) -> Optional[float]:
    ...
    for desc, grams in portions:
        if re.search(rf"^1\s*{measure}\b", desc):  # exact starts-with match
            return grams
        if measure in desc:  # loose contains
            return grams
    return None
...
def _to_grams(name: str, amt: float, unit: str, portions: List[Tuple[str, float]]) -> Optional[float]:
    ...
    if unit in ("half", "1/2"):
        one_g = _best_each_grams(name, portions)
        if one_g: return float(amt) * 0.5 * one_g
    return None
